...
#include "config.h"
#include "GStreamerIceBackendRice.h"
...
struct RecvSourceData {
    GStreamerIceBackendRice* backend;
    unsigned streamId;
};
...
GRefPtr<GSource> GStreamerIceBackendRice::getRecvSourceForStream(unsigned streamId)
{
    return m_sockets.get(streamId).source;
}

void GStreamerIceBackendRice::notifyIncomingData(unsigned streamId, WebCore::RTCIceProtocol protocol, String&& from, String&& to, std::span<const uint8_t>&& data)
{
    callOnMainRunLoopAndWait([&, streamId, protocol, data = WTFMove(data), from = WTFMove(from), to = WTFMove(to)] {
        connection()->send(Messages::GStreamerIceBackendProxy::NotifyIncomingData { streamId, protocol, WTFMove(from), WTFMove(to), WTFMove(data) }, destination());
    });
}
...
struct ResolveAddressData {
    ...
    String address;
    GStreamerIceBackendRice::ResolveCallback callback;
};
...
void GStreamerIceBackendRice::resolveAddress(const String& address, CompletionHandler<void(Expected<String, WebCore::ExceptionData>&&)>&& completionHandler)
{
    ...
    g_main_context_invoke_full(m_mainContext.get(), G_PRIORITY_DEFAULT, reinterpret_cast<GSourceFunc>(+[](gpointer userData) -> gboolean {
        ...
        g_resolver_lookup_by_name_async(data->resolver.get(), data->address.utf8().data(), nullptr,
            reinterpret_cast<GAsyncReadyCallback>(+[](GResolver* resolver, GAsyncResult* result, gpointer userData) {
                ...
                if (!addresses) {
                    auto message = makeString("Unable to resolve address: "_s, WTF::unsafeSpan(error->message));
                    callOnMainRunLoopAndWait([data, message = WTFMove(message)] {
                        data->callback(makeUnexpected(ExceptionData { ExceptionCode::NetworkError, WTFMove(message) }));
                    });
...
void GStreamerIceBackendRice::sendData(unsigned streamId, WebCore::RTCIceProtocol protocol, String from, String to, std::span<const uint8_t> data)
{
    ...
    auto result = rice_sockets_send(sockets.get(), transport, riceFrom, riceTo, data.data(), data.size());
    if (result != RICE_ERROR_SUCCESS)
        g_printerr("Failed to send data to rice sockets, error code: %d\n", static_cast<uint32_t>(result));
}

void GStreamerIceBackendRice::finalizeStream(unsigned streamId)
{
    g_printerr("Finalizing stream %u\n", streamId);
...
void GStreamerIceBackendRice::gatherSocketAddresses(unsigned streamId, CompletionHandler<void(Vector<String>&&)>&& completionHandler)
{
    ...
    RiceAddress** interfaceAddresses = rice_interfaces(&totalInterfaces);
    ...
    rice_addresses_free(interfaceAddresses, totalInterfaces);

    auto source = adoptGRef(recvSourceNew());
    ...
    g_source_set_callback(source.get(), static_cast<GSourceFunc>([](auto userData) -> gboolean {
        ...
        while (true) {
            ...
            case RICE_IO_RECV_DATA: {
                ...
                sourceData->backend->notifyIncomingData(sourceData->streamId, protocol, WTFMove(from), WTFMove(to), unsafeMakeSpan(data, recv.data.len));
                break;
            }
...
const RiceAddress* GStreamerIceBackendRice::ensureRiceAddressFromCache(const String& address)
{
