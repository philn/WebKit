...
static Expected<CandidateAddress, ExceptionData> getCandidateAddress(StringView candidate)
{
    ...
    result.prefix = prefixBuilder.toString().trim([](auto c) { return c == ' '; });

    StringBuilder suffixBuilder;
    for (unsigned i = 5; i < tokens.size(); i++)
        suffixBuilder.append(tokens[i], ' ');
    result.suffix = suffixBuilder.toString().trim([](auto c) { return c == ' '; });
    return result;
}
...
void webkitGstWebRTCIceAgentLocalCandidateGatheredForStream(WebKitGstIceAgent* agent, unsigned streamId, RiceAgentGatheredCandidate& gatheredCandidate)
{
    ...
    // When receiving a TCP candidate, notify it only if it is on Discard (9) port. Covered by test
    // webrtc/datachannel/filter-ice-candidate.html.
    if (candidate.transport_type == RICE_TRANSPORT_TYPE_TCP && rice_address_get_port(candidate.address) != 9)
        return;
