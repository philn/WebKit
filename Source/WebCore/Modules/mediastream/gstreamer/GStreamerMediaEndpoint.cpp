...
void GStreamerMediaEndpoint::processStatsItem(const GValue* value)
{
    ...
    if (m_peerConnectionBackend.isJSONLogStreamingEnabled()) {
        auto event = m_peerConnectionBackend.generateJSONLogEvent(m_peerConnectionIdentifier, gstStructureToJSONString(structure), false);
        m_peerConnectionBackend.emitJSONLogEvent(WTFMove(event));
    }

    if (m_isGatheringRTCLogs) {
        auto event = m_peerConnectionBackend.generateJSONLogEvent(m_peerConnectionIdentifier, gstStructureToJSONString(structure), true);
