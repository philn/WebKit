...
void GStreamerMediaEndpoint::doSetLocalDescription(const RTCSessionDescription* description)
{
    ...
    for (unsigned i = 0; i < totalMedias; i++) {
        ...
        auto msid = StringView::fromLatin1(gst_sdp_media_get_attribute_val(media, "msid"));
        if (msid.isEmpty())
            continue;

        GST_DEBUG_OBJECT(m_pipeline.get(), "Looking-up outgoing source with msid %s", msid.toStringWithoutCopying().ascii().data());
        m_unlinkedOutgoingSources.removeFirstMatching([&](auto& source) -> bool {
            ...
            if (UNLIKELY(gst_pad_is_linked(sinkPad.get()))) {
                GST_WARNING_OBJECT(m_pipeline.get(), "RealtimeMediaSource already linked.");
                return true;
            }
...
ExceptionOr<GStreamerMediaEndpoint::Backends> GStreamerMediaEndpoint::createTransceiverBackends(const String& kind, const RTCRtpTransceiverInit& init, GStreamerRtpSenderBackend::Source&& source, PeerConnectionBackend::IgnoreNegotiationNeededFlag ignoreNegotiationNeededFlag)
{
    ...
    auto msid = emptyString();
    if (!mediaStreamId.isEmpty() && !trackId.isEmpty())
        msid = makeString(mediaStreamId, ' ', trackId);
    ...
    for (auto& codec : codecs) {
        ...
        auto pt = gstStructureGet<int>(codecStructure, "payload"_s);
        GUniquePtr<GstStructure> codecData(gst_structure_new("codec-parameters", "pt", G_TYPE_UINT, static_cast<unsigned>(*pt),
    ...
    if (kind == "audio"_s) {
        ...
        GUniquePtr<GstStructure> encodingData(gst_structure_new("encoding-parameters", "encoding-name", G_TYPE_STRING, "H264", "payload", G_TYPE_INT, 97, "active", G_TYPE_BOOLEAN, TRUE, nullptr));
        GValue value = G_VALUE_INIT;
        g_value_init(&value, GST_TYPE_STRUCTURE);
        gst_value_set_structure(&value, encodingData.get());
        gst_value_list_append_value(&encodingsValue, &value);
...
ExceptionOr<GStreamerMediaEndpoint::Backends> GStreamerMediaEndpoint::addTransceiver(const String& trackKind, const RTCRtpTransceiverInit& init, PeerConnectionBackend::IgnoreNegotiationNeededFlag ignoreNegotiationNeededFlag)
{
    ...
    auto sourceType = trackKind == "audio"_s ? RealtimeMediaSource::Type::Audio : RealtimeMediaSource::Type::Video;
    return createTransceiverBackends(trackKind, init, createMutedSource(sourceType), ignoreNegotiationNeededFlag);
}

GStreamerRtpSenderBackend::Source GStreamerMediaEndpoint::createMutedSource(RealtimeMediaSource::Type sourceType)
{
    if (sourceType == RealtimeMediaSource::Type::Audio)
        return RealtimeOutgoingAudioSourceGStreamer::createMuted(m_ssrcGenerator);
    return RealtimeOutgoingVideoSourceGStreamer::createMuted(m_ssrcGenerator);
}
    ...
    gst_structure_map_in_place(result.get(), [](GQuark, GValue* value, gpointer userData) -> gboolean {
        ...
        case GST_WEBRTC_STATS_INBOUND_RTP: {
            ...
            if (!trackIdentifier.isEmpty())
                gst_structure_set(structure.get(), "track-identifier", G_TYPE_STRING, trackIdentifier.toString().utf8().data(), nullptr);
            break;
        }
        case GST_WEBRTC_STATS_OUTBOUND_RTP: {
            ...
            if (!ssrc) {
                GST_WARNING("Missing SSRC in outbound stats %" GST_PTR_FORMAT, structure.get());
                break;
            }

            auto ssrcString = makeString(*ssrc);
            GUniqueOutPtr<GstStructure> ssrcStats;
            gst_structure_get(additionalStats, ssrcString.ascii().data(), GST_TYPE_STRUCTURE, &ssrcStats.outPtr(), nullptr);
            if (!ssrcStats) {
                GST_WARNING("Missing SSRC %s in additional outbound stats %" GST_PTR_FORMAT, ssrcString.ascii().data(), additionalStats);
                break;
            }
