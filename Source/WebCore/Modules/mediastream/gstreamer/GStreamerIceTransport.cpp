...
static GstFlowReturn iceTransportHandleSample(WebKitGstIceTransport* self, GstAppSink* sink, bool isPreroll)
{
    ...
    webkitGstWebRTCIceAgentSend(agent.get(), rice_stream_get_id(riceStream.get()), RTCIceProtocol::Udp, from, to, data);
    rice_transmit_clear(&transmit);
    return GST_FLOW_OK;
}
...
static void fillCredentials(const GRefPtr<RiceStream>& stream [[maybe_unused]], bool isLocal [[maybe_unused]], GstWebRTCICECandidateStats* stats [[maybe_unused]])
{
#if GST_CHECK_VERSION(1, 27, 0)
    ...
    //credentials->user_name_fragment =
#endif
}
