    ...
    : m_rtcTransceiver(WTFMove(rtcTransceiver))
{
    ...
    std::call_once(debugRegisteredFlag, [] {
        GST_DEBUG_CATEGORY_INIT(webkit_webrtc_transceiver_debug, "webkitwebrtcrtptransceiver", 0, "WebKit WebRTC RTP transceiiver");
    });
...
ExceptionOr<void> GStreamerRtpTransceiverBackend::setCodecPreferences(const Vector<RTCRtpCodecCapability>& codecs)
{
    ...
    if (gst_caps_get_size(currentCaps.get()) > 0) {
        ...
        gst_structure_foreach(structure, [](GQuark name, const GValue* value, gpointer userData) -> gboolean {
            auto key = StringView::fromLatin1(g_quark_to_string(name));
            if (!key.startsWith("extmap-"_s))
                return TRUE;

            auto extensions = static_cast<HashMap<String, String>*>(userData);
