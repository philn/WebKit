...
void GStreamerIceTransportBackend::stateChanged() const
{
    ...
    bool trackReady = GPOINTER_TO_INT(g_object_get_qdata(G_OBJECT(m_backend.get()), iceTransportTrackReadyQuark()));
    GST_DEBUG_OBJECT(m_backend.get(), "Track is ready: %d", trackReady);

    if (!trackReady && transportState == GST_WEBRTC_ICE_CONNECTION_STATE_CONNECTED) {
        GST_DEBUG_OBJECT(m_backend.get(), "Delaying ICE connected state notification until track is ready");
        return;
    }
