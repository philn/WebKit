...
static gboolean webkitGstWebRTCIceStreamGatherCandidates(GstWebRTCICEStream* ice)
{
    ...
    auto error = rice_component_gather_candidates(component.get(), riceAddressValues.size(), addressDataStorage.data(), riceTransportStorage.data());
...
void webkitGstWebRTCIceStreamHandleIncomingData(WebKitGstIceStream* stream, WebCore::RTCIceProtocol protocol, String&& from, String&& to, std::span<const uint8_t>&& data)
{
    ...
    RiceStreamIncomingData ret;

    // FIXME: un-hardcode this?
    size_t componentId = 1;
    rice_stream_handle_incoming_data(stream->priv->riceStream.get(), componentId, transport, riceFrom.get(),
        riceTo.get(), data.data(), data.size(), now.nanoseconds(), &ret);

    if (ret.handled) {
        auto agent = stream->priv->agent.get();
        // may result in either the gather or conncheck sources making further
        // progress
        if (agent) [[likely]]
            webkitGstWebRTCIceAgentWakeup(agent.get());
    }
    ...
    if (ret.data.size > 0 && ret.data.ptr) {
        auto buffer = adoptGRef(gst_buffer_new_memdup(ret.data.ptr, ret.data.size));
