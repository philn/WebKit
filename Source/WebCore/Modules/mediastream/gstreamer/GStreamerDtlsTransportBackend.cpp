...
#endif
        bool trackReady = GPOINTER_TO_INT(g_object_get_qdata(G_OBJECT(m_backend.get()), dtlsTransportTrackReadyQuark()));
        GST_DEBUG_OBJECT(m_backend.get(), "Track is ready: %d", trackReady);
        if (!trackReady && state == GST_WEBRTC_DTLS_TRANSPORT_STATE_CONNECTED) {
            GST_DEBUG_OBJECT(m_backend.get(), "Delaying DTLS connected state notification until track is ready");
            return;
        }
