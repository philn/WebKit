...
typedef struct _RTPPlayoutDelayHeaderExtensionPrivate {
} RTPPlayoutDelayHeaderExtensionPrivate;

typedef struct _RTPPlayoutDelayHeaderExtension {
    GstRTPHeaderExtension parent;
    RTPPlayoutDelayHeaderExtensionPrivate* priv;
} RTPPlayoutDelayHeaderExtension;

typedef struct _RTPPlayoutDelayHeaderExtensionClass {
    GstRTPHeaderExtensionClass parentClass;
} RTPPlayoutDelayHeaderExtensionClass;

GST_DEBUG_CATEGORY_STATIC(extensionDebug);
#define GST_CAT_DEFAULT extensionDebug

WEBKIT_DEFINE_TYPE_WITH_CODE(RTPPlayoutDelayHeaderExtension, webkit_gst_rtp_playout_delay_header_extension, GST_TYPE_RTP_HEADER_EXTENSION, GST_DEBUG_CATEGORY_INIT(extensionDebug, "webkitrtpplayoutdelay", 0, "RTP Playout Delay Extension"))
...
{
    // auto [rotation, isMirrored] = webkitGstBufferGetPlayoutDelay(const_cast<GstBuffer*>(inputBuffer));
    gssize written = 3;
...
static void webkit_gst_rtp_playout_delay_header_extension_class_init(RTPPlayoutDelayHeaderExtensionClass* klass)
{
