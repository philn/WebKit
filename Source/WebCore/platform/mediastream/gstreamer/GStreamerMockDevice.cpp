...
static GstElement* webkitMockDeviceCreateElement(GstDevice* device, const char* name)
{
    ...
    auto* appSrc = makeGStreamerElement("appsrc", name);
...
GstDevice* webkitMockDeviceCreate(CaptureDevice&& captureDevice)
{
    ...
    GUniquePtr<GstStructure> properties(gst_structure_new_empty("webkit-mock-device"));
    ...
    device->priv->captureDevice = WTFMove(captureDevice);
