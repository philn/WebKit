...
bool DesktopPortalCamera::accessCamera()
{
    ...
    StringView connectionString { uniqueName, static_cast<unsigned>(strlen(uniqueName)) };
...
Vector<PipeWireNodeData> DesktopPortalCamera::openCameraPipewireRemote()
{
    ...
    auto pipeWireSession = PipeWireSession::create(fd);
    if (!pipeWireSession) {
        WTFLogAlways("Unable to connect to PipeWire");
        return { };
    }
    return pipeWireSession->run();
}
...
std::optional<PipeWireNodeData> DesktopPortalScreenCast::ScreencastSession::openPipewireRemote()
{
    ...
    // FIXME: Hook into PipeWireSession?
    ...
    //     return { };
    // }
    // auto nodes = pipeWireSession->run();
    // for (auto& node: nodes) {
    //     WTFLogAlways("---> %s", node.)
    // }
    // return nodes[0];
