...
void GStreamerAudioCapturer::setSinkAudioCallback(SinkAudioDataCallback&& callback)
{
    ...
    auto handler = g_signal_connect_swapped(sink(), "new-sample", G_CALLBACK(+[](GStreamerAudioCapturer* capturer, GstElement* sink) -> GstFlowReturn {
        ...
        // GStreamerAudioData data(WTFMove(gstSample), info);
        capturer->m_sinkAudioDataCallback.second(WTFMove(gstSample), WTFMove(presentationTime));
        return GST_FLOW_OK;
    }), this);
    m_sinkAudioDataCallback.first = handler;
}
