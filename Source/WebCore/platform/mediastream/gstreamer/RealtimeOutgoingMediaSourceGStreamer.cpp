...
RealtimeOutgoingMediaSourceGStreamer::RealtimeOutgoingMediaSourceGStreamer(Type type, const RefPtr<UniqueSSRCGenerator>& ssrcGenerator)
    ...
    , m_ssrcGenerator(ssrcGenerator)
{
    // FIXME(philn): This constructor's usage needs to be revisited. Using it leads to no mediastreamsrc element created. Seems needed to created muted sources.
    initialize();
    m_liveSync = gst_element_factory_make("identity", nullptr);
...
bool RealtimeOutgoingMediaSourceGStreamer::configurePacketizers(GRefPtr<GstCaps>&& codecPreferences)
{
    ...
    if (!gst_pad_is_linked(teeSinkPad.get()) && !gst_element_link_many(m_liveSync.get(), m_preProcessor.get(), m_tee.get(), nullptr)) {
    ...
    appendExtraRTPHeaderExtensions(structure, lookupResults.lastIdentifier);
