...
static gssize webkitGstRTPVideoRotationHeaderExtensionWrite(GstRTPHeaderExtension* extension, const GstBuffer* inputBuffer, GstRTPHeaderExtensionFlags, GstBuffer*, guint8* data, gsize)
{
    ...
    GST_TRACE_OBJECT(extension, "Wrote %" G_GSSIZE_FORMAT " bytes from video rotation %d (flipped: %s) to byte 0x%x", written, rotation, WTF::boolForPrinting(isMirrored), data[0]);
    return written;
}
...
static gboolean webkitGstRTPVideoRotationHeaderExtensionRead(GstRTPHeaderExtension* extension, GstRTPHeaderExtensionFlags, const guint8* data, gsize, GstBuffer* buffer)
{
    ...
    GST_TRACE_OBJECT(extension, "Read byte 0x%x to video rotation %d (flipped: %s)", firstByte, rotation, boolForPrinting(isMirrored));
    auto modifiedBuffer = webkitGstBufferSetVideoFrameMetadata(GRefPtr(buffer), { }, rotation, isMirrored);
    gst_buffer_replace(&buffer, modifiedBuffer.get());
    return TRUE;
}
