...
public:
    ...
    {
        return webrtc::make_ref_counted<GStreamerEncodedImageBuffer>(WTFMove(data));
    }
...
public:
    ...
    {
        auto encodedData = GStreamerEncodedImageBuffer::create(frame.data.span());

        webrtc::EncodedImage encodedFrame;
        encodedFrame.SetEncodedData(encodedData);
        if (!encodedFrame.size()) [[unlikely]]
            return;

        encodedFrame._encodedWidth = m_size.width();
        encodedFrame._encodedHeight = m_size.height();
        encodedFrame.SetFrameType(frame.isKeyFrame ? webrtc::VideoFrameType::kVideoFrameKey : webrtc::VideoFrameType::kVideoFrameDelta);
        encodedFrame.SetPresentationTimestamp(webrtc::Timestamp::Millis(frame.timestamp));

        if (auto rtpTimestamp = m_rtpTimestamps.takeOptional(frame.timestamp)) [[likely]]
            encodedFrame.SetRtpTimestamp(*rtpTimestamp);

        if (m_codecInfo.codecType == webrtc::kVideoCodecVP8)
            m_codecInfo.codecSpecific.VP8.temporalIdx = frame.temporalIndex.value_or(0);

        m_imageReadyCb->OnEncodedImage(encodedFrame, &m_codecInfo);
    }
...
std::vector<webrtc::SdpVideoFormat> GStreamerVideoEncoderFactory::GetSupportedFormats() const
{
    ...
    if (scanner.isCodecSupported(GStreamerRegistryScanner::Configuration::Encoding, "avc1"_s))
        supportedCodecs.push_back(webrtc::SdpVideoFormat::H264());
